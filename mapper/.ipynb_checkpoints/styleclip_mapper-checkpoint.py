...
class StyleCLIPMapper(nn.Module):
	...
	def __init__(self, opts):
		...
		self.decoder = Generator(self.opts.stylegan_size, 512, 8)  # stylegan_size = 1024.
...
class AU2W_Mapper(nn.Module):
	'''
		This module is to mapping src_au to the residual of W code.
	'''
	def __init__(self, opts):
		super(AU2W_Mapper, self).__init__()
		self.opts = opts
		# Define architecture
		self.mapper = self.set_mapper()
		self.decoder = Generator(self.opts.stylegan_size, 512, 8)  # stylegan_size = 1024.
	...
	def set_mapper(self):
		if self.opts.work_in_stylespace:
			mapper = AU_mapper.WithoutToRGBStyleSpaceMapper(self.opts)
		elif self.opts.mapper_type == 'SingleMapper':
			mapper = AU_mapper.SingleMapper(self.opts)
		elif self.opts.mapper_type == 'LevelsMapper':
			mapper = AU_mapper.LevelsMapper(self.opts)
		else:
			raise Exception('{} is not a valid mapper'.format(self.opts.mapper_type))
		return mapper
